...
- **Admin UI: `public/admin.html`** → https://portfolio-beta-seven-lona37wqbj.vercel.app/admin.html. Static page, supabase-js UMD from jsdelivr, anon key embedded (public by design). Email+password login (Supabase Auth; signups disabled, single owner user). CRUD on `projects`, uploads to `project-images` with public URLs. Writes gated by RLS policies `Owner manages projects` / `Owner uploads project images` via `public.is_owner()` (matches `auth.jwt()->>'email'`).
- Or edit in the Supabase dashboard Table Editor; set `status='published'` to show. Image URLs must be absolute (`safeLink` drops relative paths): upload to `project-images` bucket, use the public URL `https://nrgwvywbzesljpyyrnfi.supabase.co/storage/v1/object/public/project-images/<file>`.
