...
try {
  ...
  const audioTime=await page.locator('.short-intro').evaluate(v=>{if(v.muted||v.paused)throw Error('Short intro audio must continue');return v.currentTime;});
  await page.waitForTimeout(500);
  assert.ok(await page.locator('.short-intro').evaluate(v=>v.currentTime)>audioTime);
  assert.equal(await page.locator('.intro-video').first().evaluate(v=>v.paused),true);
  ...
  console.log(JSON.stringify({passed:true,checks:['unmuted long playback','natural long completion','skip switches to short','second scroll reveals desktop with continuing unmuted audio','long audio paused','scroll switches to short','release resets both bottom corners','yaw without roll','upward pitch','mobile swipe intro']}));
